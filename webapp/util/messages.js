jQuery.sap.declare("Z_HR_CONTACT.util.messages");
jQuery.sap.require("sap.ca.ui.message.message");

Z_HR_CONTACT.util.messages = {};
 ...
 * @public
 */
Z_HR_CONTACT.util.messages.showErrorMessage = function(oParameter) {
	var oErrorDetails = Z_HR_CONTACT.util.messages._parseError(oParameter);
...
Z_HR_CONTACT.util.messages.getErrorContent = function(oParameter) {
	return Z_HR_CONTACT.util.messages._parseError(oParameter).sMessage;
};

Z_HR_CONTACT.util.messages._parseError = function(oParameter) {
	var sMessage = "",
		sDetails = "",
		oEvent = null,
		oResponse = null,
	...
	if (oParameter.mParameters.response) {
		sMessage = oParameter.mParameters.response.message;
		sDetails = oParameter.mParameters.response.responseText;
	} else if (oParameter.mParameters) {
		oEvent = oParameter;
